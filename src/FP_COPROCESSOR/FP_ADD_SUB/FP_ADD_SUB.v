...
//altfp_add_sub CBX_AUTO_BLACKBOX="ALL" DENORMAL_SUPPORT="NO" DEVICE_FAMILY="Cyclone IV GX" DIRECTION="VARIABLE" OPTIMIZE="SPEED" PIPELINE=14 REDUCED_FUNCTIONALITY="NO" WIDTH_EXP=8 WIDTH_MAN=23 add_sub clock dataa datab nan overflow result underflow zero
...
module  FP_ADD_SUB_altbarrel_shift_t8e
	( 
	aclr,
	clk_en,
	clock,
...
endmodule //FP_ADD_SUB_altbarrel_shift_t8e
...
`timescale 1 ps / 1 ps
//synopsys translate_on
module  FP_ADD_SUB_altbarrel_shift_3cg
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	assign
		...
		smux_w = {((({26{(sel_w[4] & (~ dir_w[4]))}} & {sbit_w[113:104], pad_w[15:0]}) | ({26{(sel_w[4] & dir_w[4])}} & {pad_w[15:0], sbit_w[129:120]})) | ({26{(~ sel_w[4])}} & sbit_w[129:104])), ((({26{(sel_w[3] & (~ dir_w[3]))}} & {sbit_w[95:78], pad_w[7:0]}) | ({26{(sel_w[3] & dir_w[3])}} & {pad_w[7:0], sbit_w[103:86]})) | ({26{(~ sel_w[3])}} & sbit_w[103:78])), ((({26{(sel_w[2] & (~ dir_w[2]))}} & {sbit_w[73:52], pad_w[3:0]}) | ({26{(sel_w[2] & dir_w[2])}} & {pad_w[3:0], sbit_w[77:56]})) | ({26{(~ sel_w[2])}} & sbit_w[77:52])), ((({26{(sel_w[1] & (~ dir_w[1]))}} & {sbit_w[49:26], pad_w[1:0]}) | ({26{(sel_w[1] & dir_w[1])}} & {pad_w[1:0], sbit_w[51:28]})) | ({26{(~ sel_w[1])}} & sbit_w[51:26])), ((({26{(sel_w[0] & (~ dir_w[0]))}} & {sbit_w[24:0], pad_w[0]}) | ({26{(sel_w[0] & dir_w[0])}} & {pad_w[0], sbit_w[25:1]})) | ({26{(~ sel_w[0])}} & sbit_w[25:0]))};
endmodule //FP_ADD_SUB_altbarrel_shift_3cg
...
`timescale 1 ps / 1 ps
//synopsys translate_on
module  FP_ADD_SUB_altfp_add_sub_39n
	( 
	add_sub,
	clock,
	dataa,
...
`endif
	...
	FP_ADD_SUB_altbarrel_shift_t8e   lbarrel_shift
	( 
	.aclr(aclr),
	.clk_en(clk_en),
	.clock(clock),
	...
	FP_ADD_SUB_altbarrel_shift_3cg   rbarrel_shift
	( 
	.aclr(aclr),
	.clk_en(clk_en),
	.clock(clock),
...
endmodule //FP_ADD_SUB_altfp_add_sub_39n
//VALID FILE
...
module FP_ADD_SUB (
	...
	FP_ADD_SUB_altfp_add_sub_39n	FP_ADD_SUB_altfp_add_sub_39n_component (
				.clock (clock),
				.datab (datab),
				.dataa (dataa),
				.add_sub (add_sub),
